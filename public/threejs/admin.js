...
fetch("/api/view/" + id)
  ...
  .then((data) => {
    ...
    data.map(function (item) {
      ...
      backgroundLoad.load(item.image, function (texture) {
      ...
      document.addEventListener("mousewheel", on_mouse_wheel, true);
      ...
      fetch("/api/point/" + item.ID)
        ...
        .then((eachPoint) => {
          ...
          pointList.map(function (eachPoint) {
            ...
            add_pointer(
              new THREE.Vector3(eachPoint.x, eachPoint.y, eachPoint.z),
              eachPoint.name,
              eachPoint.href,
              eachPoint.info,
              eachPoint.type
      ...
      fetch("/api/3dmodel/" + item.ID)
        ...
        .then((data) => {
          ...
          list3D.map(function (each3D) {
            ...
            add_pointer_3D(
              new THREE.Vector3(each3D.x, each3D.y, each3D.z),
              each3D.name,
              each3D.embedded,
              "3D"
            );
...
function add_pointer(position, name, href, description, type) {
  ...
  let model = new GLTFLoader().load(
    "/texture/lowpoly_earth/scene.gltf",
    function (gltf) {
  ...
  btnHide.addEventListener("click", () => {
    if (spriteVisible == true) {
      ...
      btnHide.innerHTML =
        '<i id="iconElement" class="fa-solid fa-eye"></i>&nbsp;Hiện Icon';
    } else {
      sprite.visible = true;
      for (var i = 0; i < pointerGroup.length; i++) {
        var object = pointerGroup[i];
      ...
      btnHide.classList.add("activate");
      btnHide.innerHTML =
        '<i id="iconElement" class="fa-solid fa-eye-slash"></i>&nbsp;Ẩn Icon';
    }
  });
  //-------------------------------------------------------
}
function add_pointer_3D(position, name, embedded, type) {
  ...
  let model = new GLTFLoader().load(
    "/texture/cat_box_meme/scene.gltf",
    function (gltf) {
      let object = gltf.scene;
...
function on_mouse_wheel(event) {
  if (event.target.closest(".web-gl")) {
    //ngăn zoom ảnh khi rê chuột đến phẩm tử thông tin
...
function on_mouse_move_3D(e) {
  ...
  intersects.forEach(function (intersects) {
    if (
      intersects.object.type === "Sprite" &&
      intersects.object.typePoint != "3D"
    ) {
      let p = intersects.object.position.clone().project(camera);
      point.style.top = ((-1 * p.y + 1) * window.innerHeight) / 2 + "px";
      point.style.left = ((p.x + 1) * window.innerWidth) / 2 + "px";
      ...
      point.innerHTML =
        ...
        "<br>" +
        intersects.object.description +
        "<br>" +
        intersects.object.typePoint;
      pointActive = true;
      foundSprite = true;
    }
  });
